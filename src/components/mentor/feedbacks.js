...
const FeedB = (dqust) => {
  ...
  const Quest = () => {
    ...
    Axios.post(`https://backend-23.herokuapp.com/mentor/create-feedback/${id_quest}`, data, {
      headers: {
    ...
    <div>
      ...
      <Modal isOpen={modalIsOpen} onAfterOpen={afterOpenModal} onRequestClose={closeModal} style={customStyles} contentLabel="Example Modal">
        <ThemeProvider theme={theme}>
          ...
          <input name="konten" value={konten} onChange={(e) => setKonten(e.target.value)} /> <Button onClick={Quest}>Komentar</Button>
        </ThemeProvider>
      </Modal>
    </div>
  );
